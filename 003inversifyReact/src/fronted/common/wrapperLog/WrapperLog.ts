...
@injectable()
export class WrapperLog implements IWrapperLog {
  private logConf: string = "";
  private bizName: string = "*";
  private targetBizName: string = "*";
  private targetLevel: string = "warn";

  @postConstruct()
  public init() {
    const _location = location || ({} as any);
    this.logConf = (/__(?:logConf|logLevel)__=([^#/&]*)/.exec(_location.href) ||
      [])[1];
    // 设置默认的bizName为调试模式
    this.setOptions({ level: "debug",
      bizName: "*",});
  }

  public setOptions(options: ILogOptions) {
    options = { ...defaultOptions, ...options };
    const targetOptions = parseLogConf(this.logConf, options);
